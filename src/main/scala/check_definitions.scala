...
object check_definitions {

  /** A wrapper for checks to be applied to a tuple of fields. This is a workaround for the fact that we cannot hold type-constructors in a tuple, but must have
    * a fully saturated type instead.
    */
  ...
  type ApplyCheck[Fields <: Tuple] = [C] =>> C match
