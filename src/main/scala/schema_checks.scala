...
import case_class_definitions.*
import data_types.*
import helpers.*

object schema_checks {

  type SchemaChecks =
    HasAtMostFields2[25] *: ExactlyOneOf[Key, "Key"] *: ExactlyOneOf[Timestamp, "Timestamp"] *: StringFieldsAtMost[500] *: HasPrefix["tr"] *: EmptyTuple
  ...
  type HasAtMostFields[MaxSize <: Int] = Check[
    ...
    (Size[Fields] <= MaxSize) OrFailWith "Too many fields in class: " ++ ToString[Size[Fields]] ++ " > " ++ ToString[MaxSize],
  ]
  ...
  type HasAtMostFields2[MaxSize <: Int] = Check[
    [Fields <: Tuple] =>> Size[Fields] match
      case Is[size] =>
        (size <= MaxSize) OrFailWith "Too many fields in class: " ++ ToString[size] ++ " > " ++ ToString[MaxSize],
  ]

  type ExactlyOneOf[Type, TypeName <: String] = Check[
    [Fields <: Tuple] =>> CollectFieldsOfType[Type, Fields] match
      case EmptyTuple      => Error["No [" ++ TypeName ++ "] field in class"]
      case h *: EmptyTuple => Okay
      case h *: t          =>
        Error[
          "Expected exactly one [" ++ TypeName ++ "] " ++ "field in class, but got " ++ ToString[Size[h *: t]] ++ ": " ++ "[" ++ RenderFieldList[h *: t] ++ "]",
        ],
  ]
  ...
  type StringFieldsAtMost[MaxSize <: Int] = CheckAllFields[
    [Field] =>> GetFieldType[Field] match
      case SizedString[n] =>
        (n <= MaxSize) OrFailWith "string is too large: " ++ ToString[n] ++ " > " ++ ToString[MaxSize]
      case String         =>
  ...
  type HasPrefix[Prefix <: String] = CheckAllFields[
    [Field] =>> Matches[GetFieldName[Field], Prefix ++ ".*"] OrFailWith "field is missing prefix [" ++ Prefix ++ "]",
  ]
